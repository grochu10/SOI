 ...
 */
  ...
  if(fit == 0){
	...
	do{
	      hp = max_hole = hole_head;
	      prev_max = prev_ptr = NIL_HOLE;
	      ...
	      {
			/*adres alloca przypisany na max_hole*/
			old_base = hp->h_base;	/* remember where it started */
			hp->h_base += clicks;	/* bite a piece off */
			hp->h_len -= clicks;	/* ditto */
			/* Delete the hole if used up completely. */
...
int do_hole_map()
{
	...
	buf[i] = 0;/*0 na koniec bufora*/
	 sys_copy(0,D,(phys_bytes) buf, usr_pid, D, (phys_bytes) usr_buffer, (i+1)*sizeof(phys_clicks));
  /*wynik - liczba par w buforze*/
  return (i/2);	 
}
/*===========================================================================*
 *				do_worst_fit				     *
...
int do_worst_fit()
{
  if(mm_in.m1_i1 == 0 || mm_in.m1_i1 == 1)fit = mm_in.m1_i1;
  return 0;
}
