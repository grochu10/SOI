...
PRIVATE void sched()
{
  register struct proc *check, *act;

  /*czy mamy jakis proces uzytkownika */
  if (rdy_head[USER_Q] == NIL_PROC) return;

  act = rdy_head[USER_Q];/* wskaznik na aktualnie wywlaszczony proces */
  act -> counter = 0; /* wyzerowanie licznika zaglodzenia */
  act = act->p_nextready;
  while( act != NIL_PROC ){
        act->counter++;
	act = act -> p_nextready;
  }/*przejscie po liscie procesow i podniesienie stanow licznika */

  check = rdy_head[USER_Q];

  /* przeniesienie procesu na koniec kolejki */
  ...
  /* sprawdzenie czy kolejka przeszla prawidlowo i czy kolejny proces czeka*/
  if((act == NIL_PROC) || (act -> p_nextready == NIL_PROC)||(act -> p_nextready == check))return;

  while((act -> p_subprior == SPRIO_BACK) && (act -> counter < COUNTER_MAX))
  {
      rdy_tail[USER_Q]->p_nextready = rdy_head[USER_Q];
      rdy_tail[USER_Q] = rdy_head[USER_Q];
      rdy_head[USER_Q] = rdy_head[USER_Q]->p_nextready;
