...
FORWARD _PROTOTYPE( int do_setprio, (message *m_ptr) ); /*SOI LAB2 */
...
{
	struct proc *pp;
	pp = proc_addr(m_ptr -> m1_i1);
	/*czy proces nalezy do odpowiedniego adresu*/
	if((pp < BEG_USER_ADDR)||(pp >= END_PROC_ADDR))
		return ESRCH;
	/*czy jest blad podpriorytetu*/
	if(((m_ptr -> m1_i2) != SPRIO_OFF) && ((m_ptr -> m1_i2) != SPRIO_NORM)&&((m_ptr -> m1_i2) != SPRIO_BACK))
	    return(EINVAL);
	pp -> p_subprior = m_ptr ->m1_i2;/*nadanie podpriorytetu*/
	return(0);
}
